# IIS Deployment Script with Custom Port Selection
...
Write-Host "Default port 80 is typically used for web servers." -ForegroundColor Gray
Write-Host "If port 80 is in use, choose an alternative port (e.g., 8080, 5000, 9000)" -ForegroundColor Gray
Write-Host ""
$portInput = Read-Host "Enter port number (press Enter for default 8080)"
...
if ($portInUse) {
    ...
    Write-Host "  Please choose a different port." -ForegroundColor Yellow
    exit 1
} else {
...
Write-Host ""
Write-Host "IMPORTANT: Since you're using port $port, users will need to" -ForegroundColor Yellow
